...
def _GenerateXsrfToken(self, user_id):
    """Generates an XSRF token for the given user_id.
    
    Tady jakoby generuju ten token,jo, takze tak.
    """"

    secret_key = b'gdgdghdvfnjuxgvfuebuicvguibhjvgjcvbsjgfehj'
    
    # Combine the user_id and secret key to create the token
    token = hmac.new(secret_key, str(user_id).encode('utf-8'), hashlib.sha256).hexdigest()
    
    return token
"""

Tady ho jakoby porovnavam , takze snad to dela to co ma.
"""

def _VerifyXsrfToken(self, user_id, token):
    """Verifies if the provided token matches the expected XSRF token for the given user_id."""
    # Generate the expected XSRF token for the given user_id
    expected_token = self._GenerateXsrfToken(user_id)
  ...
  def _DoLogin(self, cookie, specials, params):
    ...
    self._SendTemplateResponse('/login.gtl', specials, params)
        xsrf_token = self._GenerateXsrfToken(cookie[COOKIE_UID])
  ...
  def _GetSnippets(self, cookie, specials, create=False):
    ...
    return snippets
"""
Jak by se asi mozna teoritecky vzato dalo mozna zabranovat XSRF,nez pridam snippet tak zkontorluji token a uvidime jestli je to v chillu.
Jine metody delat nebudu , python mi nejak moc nejde takze asi tak.
"""
  def _DoNewsnippet2(self, cookie, specials, params):
    """Handles the /newsnippet2 url: actually add the snippet."""
    # Verify XSRF token before adding the snippet
    if self._VerifyXsrfToken(cookie[COOKIE_UID], params.get('xsrf_token')):
        # Add the snippet
        snippet_text = params.get('snippet_text')
        snippet_owner = cookie[COOKIE_UID]
        snippet_time = time.time()
        snippet_key = self._MakeSnippetKey(snippet_owner, snippet_time)
        snippet = model.Snippet(snippet_text=snippet_text,
                                snippet_owner=snippet_owner,
        ...
        # Handle invalid or missing XSRF token
        self.error(403)
        self.response.out.write('Invalid or missing XSRF token.')
